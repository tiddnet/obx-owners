...
# smoke test against owners.obx.deals, and alerts via SES on failure.
...
SITE_URL="https://owners.obx.deals"
...
SMOKE_SCRIPT="$RI_ROOT/scripts/test_owners_site.py"
if [ ! -f "$SMOKE_SCRIPT" ]; then
    echo "smoke test script not found at $SMOKE_SCRIPT -- nothing to run yet."
    echo "(expected until rental-intel ships scripts/test_owners_site.py per ADR 0242 section 2)"
    _write_sentinel "error" "smoke test script missing: $SMOKE_SCRIPT"
    exit 0
fi

echo ""
echo "--- running scripts/test_owners_site.py against $SITE_URL ---"
SMOKE_OUTPUT="$("$PYTHON" "$SMOKE_SCRIPT" --url "$SITE_URL" 2>&1)"
SMOKE_EXIT=$?
echo "$SMOKE_OUTPUT"
...
if [ "$SMOKE_EXIT" -ne 0 ]; then
    ...
    ALERT_SUBJECT="[post-deploy] obx-owners smoke test failed after push"
    ALERT_BODY="Commit: $COMMIT
Site: $SITE_URL
Smoke test: rental-intel/scripts/test_owners_site.py
